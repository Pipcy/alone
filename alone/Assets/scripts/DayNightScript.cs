...
//     public Tilemap[] stars; // star sprites 
...
//         // {
//         //     timeDisplay.text = string.Format("{0:00}:{1:00}", 6, mins);
//         // }
//         // else if (days == 2 && hours > 7)
//         // {
...
public class DayNightScript : MonoBehaviour
{
    ...
    public void DisplayTime() // Shows time and day in ui
    {
        ...
        if (days == 2 && hours >=6)
        {
            timeDisplay.text = string.Format("{0:00}:{1:00}", 6, 00);
        }
        if (days == 2 && hours >= 7 && mins >=30)
        {
            SceneManager.LoadScene("Lost");
        }
    }
